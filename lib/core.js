...
AccountsTemplates.configureRoute = function(route, options) {
  ...
  if (layoutType === "blaze") {
    ...
    if (Package['wolas:blaze-layout']) {
      var BlazeLayout = Package['wolas:blaze-layout'].BlazeLayout;
    } else {
      throw new Error("useraccounts:flow-routing requires that your project includes wolas:blaze-layout package.");
    }
